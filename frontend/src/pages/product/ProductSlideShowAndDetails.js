...
function ProductSlideShowAndDetails() {
  ...
  const getUser = useCallback(async () => {
    const { data } = await axios.get(
      `http://localhost:3600/api/getUser/${idUser}`
    );
    setData(data);
  ...
  const showGallery = async () => {
    const response = await axios.get(
      `http://localhost:3600/gallery/showGallery/${id}`
    );
  ...
  //   const response = await axios.get(
  //     `http://localhost:3600/comment/getComments/${product._id}`
  //   );
  ...
  //     await axios.post(`http://localhost:3600/comment/addComment`, {
  //       username: idUser,
  //       product: product._id,
  //       comments: { comment },
  //     });
