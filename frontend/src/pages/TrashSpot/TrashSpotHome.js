...
function TrashSpotHome(props) {
  ...
  const [deleteRoundedAndCreateAnother, setDeleteRoundedAndCreateAnother] =
    useState([]);
    ...
    if (triggerWhichPage == 0 && filteredCopy) {
      setTrashSpots(
        filteredCopy.filter((trash) => {
          if (trash.ownerId._id != id && trash.frontEndLevel == 'primary')
            return trash;
        })
      );
    }
  ...
  const addAndUpdateUntouchableDataBackend = (trash) => {
    ...
    if (index > -1) {
      const untouchableDataBackendCopy = JSON.parse(
        JSON.stringify(untouchableDataBackend)
      );
      ...
      let untouchableDataBackendCopy = JSON.parse(
        JSON.stringify(untouchableDataBackend)
      );
  ...
  const removeTrashFromAllTrashSpots = (trash, old_id) => {
    ...
    if (trash['ownerId']) {
      if (index > -1) {
        ...
        untouchableDataBackendCopy[index] = trash;
        setUntouchableDataBackend(
          JSON.parse(JSON.stringify(untouchableDataBackendCopy))
        );
  ...
  return (
    <>
      ...
      <div className="d-flex ">
        <div
          className="leftSide bg-green-800"
          style={{ width: '710px', zIndex: 1 }}
        >
          <div className="container ">
            <div className="buttonsTrash bg-green-800">
              <button
